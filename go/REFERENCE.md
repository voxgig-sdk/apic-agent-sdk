# ApicAgent Golang SDK Reference

Complete API reference for the ApicAgent Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *ApicAgentSDK`
...
#### `ParseUserAgentGet(data map[string]any) ApicAgentEntity`

Create a new `ParseUserAgentGet` entity instance. Pass `nil` for no initial data.

#### `ParseUserAgentPost(data map[string]any) ApicAgentEntity`

Create a new `ParseUserAgentPost` entity instance. Pass `nil` for no initial data.
...
```go
result, err := client.ParseUserAgentGet(nil).Load(map[string]any{"ua": "ua"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
