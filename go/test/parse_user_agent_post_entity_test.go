...
func TestParseUserAgentPostEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		parseUserAgentPostRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "parse_user_agent_post"}), "parse_user_agent_post_ref01"))

		parseUserAgentPostRef01DataResult, err := parseUserAgentPostRef01Ent.Create(parseUserAgentPostRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		parseUserAgentPostRef01Data = core.ToMapAny(entityData(parseUserAgentPostRef01DataResult))
...
func parse_user_agent_postBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("APIC_AGENT_TEST_PARSE_USER_AGENT_POST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["APIC_AGENT_TEST_PARSE_USER_AGENT_POST_ENTID"])
