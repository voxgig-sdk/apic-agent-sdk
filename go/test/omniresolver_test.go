...
import (
	...
	omni "github.com/voxgig-sdk/apic-agent-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ApicAgentError); is {
				out := map[string]any{
					"name":    "ApicAgentError",
...
func omniCtx(arg any, client *sdk.ApicAgentSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ApicAgentSDK); is {
			client = live
