// Command ApicAgent-cli is a CLI + REPL for the ApicAgent SDK,
...
const prompt = "apic-agent"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "parse_user_agent_get parse_user_agent_post"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      APIC_AGENT_APIKEY (api key), APIC_AGENT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: APIC_AGENT_APIKEY carries the API key and
	// APIC_AGENT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("APIC_AGENT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("APIC_AGENT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewApicAgentSDK(opts)
