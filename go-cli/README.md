...
boru-driven command-line client **and** interactive REPL for the ApicAgent
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/apic-agent-cli)
make build
...
./apic-agent-cli load 1 parse_user_agent_get            # {id:1} shorthand
./apic-agent-cli load '{id:1}' parse_user_agent_get       # explicit match map

# 5. Override the API base URL for a single call
APIC_AGENT_BASE=https://api.example.com ./apic-agent-cli load 1 parse_user_agent_get

# 6. No arguments -> interactive REPL
./apic-agent-cli
apic-agent> load 1 parse_user_agent_get
apic-agent> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/apic-agent-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/apic-agent-cli load 1 parse_user_agent_get
   ```
...
```sh
./apic-agent-cli load 1 parse_user_agent_get          # scalar shorthand for {id:1}
./apic-agent-cli load '{id:1}' parse_user_agent_get     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `apic-agent>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `parse_user_agent_get`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/apic-agent-cli`. |
...
(`./apic-agent-cli <expr>`) and interactively (the REPL), and expressions compose the
