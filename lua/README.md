...
The Lua SDK for the ApicAgent API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:ParseUserAgentGet()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── apic-agent_sdk.lua    -- Main SDK module
...
The main module (`apic-agent_sdk`) exports the SDK constructor
