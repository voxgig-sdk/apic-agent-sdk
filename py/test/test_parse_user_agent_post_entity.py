    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set APIC_AGENT_TEST_PARSE_USER_AGENT_POST_ENTID JSON to run live")
        client = setup["client"]
        ...
        parse_user_agent_post_ref01_data = helpers.to_map(runner.entity_data(parse_user_agent_post_ref01_ent.create(parse_user_agent_post_ref01_data, None)))
...
def _parse_user_agent_post_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "APIC_AGENT_TEST_PARSE_USER_AGENT_POST_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("APIC_AGENT_TEST_PARSE_USER_AGENT_POST_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("APIC_AGENT_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("APIC_AGENT_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("APIC_AGENT_TEST_EXPLAIN") == "TRUE",
        "live": _live,
