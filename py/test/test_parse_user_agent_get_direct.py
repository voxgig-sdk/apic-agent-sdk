...
class TestParseUserAgentGetDirect:

    def test_should_direct_load_parse_user_agent_get(self):
        setup = _parse_user_agent_get_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-parse_user_agent_get", "live" if setup["live"] else "unit")
        if _skip:
...
def _parse_user_agent_get_direct_setup(mockres):
