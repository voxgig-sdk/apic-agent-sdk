...
module ApicAgentUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ApicAgentResponse.new({})
    ...
    if fetch_err
      ...
      response = ApicAgentResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ApicAgentResponse.new(fetched)
    else
