...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ApicAgentOmni.make_runner(TEST_JSON_FILE, ApicAgentSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ApicAgentOmni.errify(err)
    end
