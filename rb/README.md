...
The Ruby SDK for the ApicAgent API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.ParseUserAgentGet` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = ApicAgentSDK.test(testopts, sdkopts)
```
...
raise a `ApicAgentError` (a `StandardError` subclass), so wrap
...
```
rb/
├── ApicAgent_sdk.rb       -- Main SDK module
...
The main module (`ApicAgent_sdk`) exports the SDK class
