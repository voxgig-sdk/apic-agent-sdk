...
class ParseUserAgentGetEntity
  ...
  def make
    opts = @_entopts.dup
    ParseUserAgentGetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ApicAgentHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ParseUserAgentGet, Hash] the current ParseUserAgentGet data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ApicAgentHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ParseUserAgentGet fields)
  def match_get
  ...
  #
  # @param reqmatch [ParseUserAgentGetLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ParseUserAgentGet.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ParseUserAgentGet, Hash] the loaded ParseUserAgentGet; raises ApicAgentError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ApicAgentHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
