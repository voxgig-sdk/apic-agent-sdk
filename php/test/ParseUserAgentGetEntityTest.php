...
class ParseUserAgentGetEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set APIC_AGENT_TEST_PARSE_USER_AGENT_GET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function parse_user_agent_get_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("APIC_AGENT_TEST_PARSE_USER_AGENT_GET_ENTID");
