...
class FtCtrl extends ApicAgentControl
{
...
class FtClient
{
    ...
    private ApicAgentUtility $utility;

    public function __construct(ApicAgentUtility $utility, array $options)
    {
    ...
    public function get_utility(): ApicAgentUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ApicAgentContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ApicAgentContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ApicAgentContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ApicAgentConfig::make_feature($name);
    ...
    private static function buildUrl(ApicAgentSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ApicAgentError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ApicAgentResponse($fetched);
        }
    ...
    private function fail(ApicAgentContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ApicAgentContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
