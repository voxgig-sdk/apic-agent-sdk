...
class ApicAgentNetsimFeature extends ApicAgentBaseFeature
{
    ...
    private function _simulate(ApicAgentContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ApicAgentContext $ctx, array $applied): void
    {
        $client = $this->client;
