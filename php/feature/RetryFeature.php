...
class ApicAgentRetryFeature extends ApicAgentBaseFeature
{
    ...
    public function init(ApicAgentContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ApicAgentContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ApicAgentContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ApicAgentContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
