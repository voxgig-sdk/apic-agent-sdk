...
// ApicAgent SDK ParseUserAgentGet entity
...
class ParseUserAgentGetEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "parse_user_agent_get";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new ParseUserAgentGetEntity($this->_client, $opts);
    }

    /**
     * @param ParseUserAgentGet|array $args ParseUserAgentGet data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ApicAgentHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ParseUserAgentGet|array The current ParseUserAgentGet data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ParseUserAgentGet fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ApicAgentHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ParseUserAgentGet fields).
     */
    ...
    /**
     * Load a single ParseUserAgentGet.
     *
     * @param ParseUserAgentGetLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ParseUserAgentGetLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ParseUserAgentGet|array The loaded ParseUserAgentGet as an assoc-array at the
     *   SDK boundary; throws ApicAgentError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ApicAgentHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
