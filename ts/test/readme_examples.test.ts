...
//     (\`new ApicAgentSDK(...)\` / \`ApicAgentSDK.test(...)\` become
//     \`ApicAgentSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"parse_user_agent_get":{"test01":{"id":"test01"}},"parse_user_agent_post":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ApicAgentSDK(...)\` and any
// \`ApicAgentSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ApicAgentSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ApicAgentSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
