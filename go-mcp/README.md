...
[MCP](https://modelcontextprotocol.io) server exposing the ApicAgent SDK as
two agent tools — `apic-agent_list` and `apic-agent_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/apic-agent-mcp)
make build

# 2. Provide credentials via the environment
export APIC_AGENT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user apic-agent \
  -- /absolute/path/to/apic-agent-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./apic-agent-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// apic-agent_list: first page of records
{ "entity": "parse_user_agent_get" }
{ "entity": "parse_user_agent_get", "query": { } }

// apic-agent_load: one record by id
{ "entity": "parse_user_agent_get", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/apic-agent-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user apic-agent \
     -- "$PWD"/dist/*/apic-agent-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `apic-agent_list` and `apic-agent_load` tools now appear
   in new sessions. Ask the agent to *"list parse_user_agent_get using apic-agent"*
   and it calls `apic-agent_list` with `{"entity":"parse_user_agent_get"}`.
...
```sh
export APIC_AGENT_APIKEY=sk_live_xxx            # API key
export APIC_AGENT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `apic-agent_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `apic-agent_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `APIC_AGENT_APIKEY` | API key sent with every request. |
| `APIC_AGENT_BASE` | Optional override of the API base URL. |
...
```sh
./apic-agent-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"apic-agent_load","arguments":{"entity":"parse_user_agent_get","query":{"id":1}}}}'
```
